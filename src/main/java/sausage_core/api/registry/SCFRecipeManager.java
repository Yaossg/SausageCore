...
public abstract class SCFRecipeManager {
    ...
    public static <T> SCFRecipeType<T> newType(ResourceLocation name, Class<T> clazz, BiFunction<JsonContext, JsonObject, T> function, String version, String description) {
        SCFRecipeType<T> type = new SCFRecipeType<>(name, clazz, function, version, description);
        instance().types.put(type.name, type);
        return type;
    }
    public static <T> SCFRecipeType<T> get(ResourceLocation name) {
    ...
    public Map<String, Path> getDefaults(ResourceLocation name) {
        return Collections.unmodifiableMap(get(name).defaults);
    }
}
